...
\begin{frame}{Pipeline overview}
  ...
  \begin{tikzpicture}[
    ...
    \node[feat, below=1.0cm of val] (sphere) {Feature 1\\Global sphere\\(humeral head)};
    \node[feat, right=of sphere] (circles) {Feature 2\\Per-slice articular\\circles};
    \node[feat, right=of circles] (locate) {Tear localization\\(angular sector\\$\to$ tendon)};
    ...
    \draw[arr] (val) -- (resam);
    \draw[arr] (val.south) |- (sphere.north);
    \draw[arr] (resam.south) |- (circles.north);
    \draw[arr] (sphere) -- (circles);
    \draw[arr] (circles) -- (locate);
  \end{tikzpicture}
  \end{center}
